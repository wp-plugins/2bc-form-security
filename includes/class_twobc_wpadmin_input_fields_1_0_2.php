...
/*
 * Easily add input fields to WordPress admin screens
 * Copyright (C) 2014-2015 2BCoding (email : info@2bcoding.com)
 *
...
if ( !class_exists('twobc_wpadmin_input_fields_1_0_2') ) {
	/**
	 * Class twobc_wpadmin_input_fields
	 * Easily add input fields to WordPress admin screens.
	 *
	 ...
	 */
	class twobc_wpadmin_input_fields_1_0_2 {
		// class variables
		private $echo;
		private $nonce;
		private $html5;
		...
		function field($input_tag_args) {
			...
			if ( empty($input_tag_args['type']) )
				$errors[] = __('Error: type cannot be empty', '2bc-wpaf');
			...
			switch ($input_tag_args['type']) {
				...
				default :
					$errors[] = sprintf(__('Error: invalid type - %s', '2bc-wpaf'), esc_html($input_tag_args['type']));
			}

			// sanitize name
			$sanitized_name = '';
			if ( empty($input_tag_args['name']) ) {
				$errors[] = __('Error - name cannot be empty', '2bc-wpaf');
			} else {
				$sanitized_name = esc_attr($input_tag_args['name']);
				if ( empty($sanitized_name) )
					$errors[] = sprintf(__('Error - invalid name - %s', '2bc-wpaf'), esc_html($input_tag_args['name']));
			}
...
';
				...
				if ( !empty($input_tag_args['class']) ) {
					if (is_array($input_tag_args['class']) ) {
						/* foreach ($input_tag_args['class'] as &$_class_name) {
							$_class_name = sanitize_html_class($_class_name);
						} */
						$input_tag_args['class'] = array_filter($input_tag_args['class'], 'sanitize_html_class');
					...
					} elseif ( is_string($input_tag_args['class']) ) {
						$class_array = explode(' ', $input_tag_args['class']);
						$class_array = array_filter($class_array, 'sanitize_html_class');
						$input_tag_args['class'] = implode(' ', $class_array);
					}
				}
						...
						if ( !empty($_sanitized_value) ) {
							switch (true) {
								case ( is_string($_option_text) ) :
									$sanitized_options[$_sanitized_value] = wp_kses_post($_option_text);
									break;
								...
								case ( is_array($_option_text) && isset($_option_text['label']) ) :
									...
									// sanitize output
									$sanitized_options[$_sanitized_value]['label'] = wp_kses_post($_option_text['label']);
									break;
...
') . $sanitized_description;
						...
						case 'radio' :
							// nothing here, description already taken care of
							break;

						default :
							// everything else
							$returned_input_tag .= $sanitized_description;
					}
				}
				// BUILD TAG - END
